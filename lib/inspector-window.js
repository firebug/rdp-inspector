...
/** @lends InspectorWindow */
{
  ...
  initialize: function(InspectorService, toolboxOverlay, inspectorWindowName) {
    ...
    this.onWindowInitialized = this.onWindowInitialized.bind(this);
  ...
  open: function() {
    ...
    // If the Inspector is already opened, focus and return it.
    if (this.win) {
      this.win.focus();
      return this.win;
    }

    return this._open("rdp-inspector", INSPECTOR_XUL_URL, [{
      inspectorWindowName: this.inspectorWindowName
    }]);
  },

  _open: function(windowType, url, params) {
    Trace.sysout("InspectorWindow.open;", arguments);

    // Open RDP Inspector console window.
    this.win = openDialog({
      url: url,
      name: this.inspectorWindowName,
      args: params,
  ...
  onWindowInitialized: function(event) {
    ...
    this.InspectorService.getInspectorClients(this.toolbox).then(clients => {
