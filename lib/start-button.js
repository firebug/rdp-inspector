...
// RDP Inspector
const { ConnectionListWindow } = require("./connection-list-window");
...
/** @lends StartButton */
{
  ...
  initialize: function() {
    ...
    this.connectionListWindow = new ConnectionListWindow();
  },
  ...
  getMenuItems: function(/*win*/) {
    ...
    items.push({
      ...
      items: [
        {
          nol10n: true,
          label: Locale.$STR("rdpInspector.menu.autoOpenOnWebIDEConnection.label"),
          tooltiptext: Locale.$STR("rdpInspector.menu.autoOpenOnWebIDEConnection.tip"),
          type: "checkbox",
          checked: prefs.webideConnectionsMonitor,
          command: this.onToggleAutoOpenOnWebideConnections.bind(this)
        },
        {
          nol10n: true,
    ...
    items.push({
      nol10n: true,
      label: Locale.$STR("rdpInspector.menu.connectionList.label"),
      tooltiptext: Locale.$STR("rdpInspector.menu.connectionList.tip"),
      command: this.onConnectionList.bind(this)
    });
  ...
  // Commands
  onConnectionList: function(event) {
    cancelEvent(event);

    this.connectionListWindow.open();
  },

  onToggleAutoOpenOnWebideConnections: function(event) {
    cancelEvent(event);

    prefs.autoOpenOnWebIDEConnection = !prefs.autoOpenOnWebIDEConnection;
  },
