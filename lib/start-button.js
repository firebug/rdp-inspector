...
const { getMostRecentBrowserWindow, getMostRecentWindow } = require("sdk/window/utils");
...
/** @lends StartButton */
{
  ...
  onToggleRdpInspector: function() {
    ...
    return getToolboxWhenReady().then(toolbox => {
      ...
      let url = self.data.url("inspector-console.html");
      let win = this.openInspector("rdp-inspector", url);
  ...
  openInspector: function(windowType, url, params) {
    ...
    let parentWindow = (!browser.opener || browser.opener.closed) ?
